...
Se breve: un veredicto por criterio y el motivo en una sola frase.
...
{ "verdicts": [ { "criterio": "accionable" | "fundado" | "seguro" | "claro", "verdict": "pass" | "fail", "motivo": "1 frase si falla" } ], "riesgo": "bajo" | "medio" | "alto" }`;
  ...
  const { res } = await generateConFallback(ai, {
    contents: [
      { role: "user", parts: [{ text: rubrica }, { text: `\nArtefacto:\n${JSON.stringify({ resumen: data.resumen, diagnostico: data.diagnostico, ruta: (data.ruta || []).slice(0, 6), drills: data.drills }).slice(0, 5000)}` }] },
    ],
