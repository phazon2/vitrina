...
function pasarAlSiguienteModelo(err) {
  ...
  return /50[0-9]|429|UNAVAILABLE|RESOURCE_EXHAUSTED|high demand|overloaded/i.test(s)
    || /40[34]|PERMISSION_DENIED|NOT_FOUND|does not have permission|is not found/i.test(s);
}
...
export function mensajeUsuario(err) {
  const s = String((err && err.message) || err);
  if (/PERMISSION_DENIED|403|does not have permission/i.test(s)) {
    return "No pudimos leer tu documento con el motor de IA. Prueba con una foto del documento en vez del PDF, o escribenos por WhatsApp y lo procesamos a mano.";
  }
  ...
  if (/50[0-9]|UNAVAILABLE|overloaded/i.test(s)) {
    return "El servicio de IA no respondio a tiempo. Intenta de nuevo en un momento.";
  }
  if (/timeout|abort/i.test(s)) {
    return "Tu documento tardo demasiado en procesarse. Prueba con un archivo mas liviano o con una foto.";
  }
  if (/JSON|parse|Respuesta vacia|no contiene/i.test(s)) {
    return "La respuesta llego incompleta. Intenta de nuevo: suele funcionar al segundo intento.";
  }
